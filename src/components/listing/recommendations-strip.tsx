...
interface RecommendationsStripProps {
  mode: 'listing' | 'personalized' | 'popular';
  listingSlug?: string;
}
...
function CompactCard({ listing }: CompactCardProps) {
...
const TITLES: Record<RecommendationsStripProps['mode'], string> = {
  listing: 'مشابه لما شاهدته مؤخراً',
  personalized: 'عقارات قد تعجبك',
  popular: 'الأكثر مشاهدة',
};

export function RecommendationsStrip({ mode, listingSlug }: RecommendationsStripProps) {
  ...
  useEffect(() => {
    ...
    if (mode === 'listing' && listingSlug) {
      url = `/api/recommendations?slug=${encodeURIComponent(listingSlug)}`;
    ...
    // mode === 'popular' uses the base URL with no params

    fetch(url, { headers })
      .then(async (res) => {
        if (!res.ok) return;
        const json = (await res.json()) as { success: boolean; data?: ListingCard[] };
        if (json.success && Array.isArray(json.data) && json.data.length > 0) {
          setListings(json.data.slice(0, 4));
        }
      })
      .catch(() => {
        // Silently fail — don't show empty section
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  return (
    <section className="py-8" dir="rtl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-lg font-bold text-gray-900 mb-4">{TITLES[mode]}</h2>
